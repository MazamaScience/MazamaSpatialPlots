...
monitorMaxDayData <- neMonitor %>% 
  monitor_dailyStatistic() %>%        # average by day 
  monitor_extractData() %>% 
  tidyr::pivot_longer(-datetime, names_to = "monitorID", values_to = "value") %>%
  dplyr::group_by(monitorID) %>% 
  dplyr::summarise(monitorPMValue = max(value, na.rm=T)) %>%  # maximum daily average
...
# --------- Prepare SPDF for spatial points
