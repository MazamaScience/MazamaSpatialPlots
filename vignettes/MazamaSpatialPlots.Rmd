...
**MazamaSpatialPlots** equires spatial polygon data to plot 
...
User data consists of simple dataframes. For the `stateMap()` function, the 
input dataframe must have a `stateCode` column containing the 2-character US
state code. Any other column of data can be used as the `parameter` states will
be colored by.
...
For the `countyMap()` example, we use COVID19 case data from June 12, 2020.
